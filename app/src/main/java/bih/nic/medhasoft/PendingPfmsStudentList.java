...
public class PendingPfmsStudentList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dataBaseHelper = new DataBaseHelper(PendingPfmsStudentList.this);

        pd1 = new ProgressDialog(PendingPfmsStudentList.this);
...
//        img_search=(ImageView)findViewById(R.id.img_search);
        ...
        Userid = PreferenceManager.getDefaultSharedPreferences(PendingPfmsStudentList.this).getString("UserId", "");
        try {
            blockcode = PreferenceManager.getDefaultSharedPreferences(PendingPfmsStudentList.this).getString("Block", "");
            distcode = PreferenceManager.getDefaultSharedPreferences(PendingPfmsStudentList.this).getString("District", "");
            panchyatcode = PreferenceManager.getDefaultSharedPreferences(PendingPfmsStudentList.this).getString("PanchayatCode", "");
            DistName = PreferenceManager.getDefaultSharedPreferences(PendingPfmsStudentList.this).getString("DistrictName", "");
            blockName = PreferenceManager.getDefaultSharedPreferences(PendingPfmsStudentList.this).getString("BlockName", "");
            PanchayatName = PreferenceManager.getDefaultSharedPreferences(PendingPfmsStudentList.this).getString("PanchayatName", "");
...
//                    new LoadAttendance(_diseCode,_varClass_Id).execute();
    ...
    {
        localDB = new DataBaseHelper(PendingPfmsStudentList.this);
        ...
        if(ClassList.size()>0 ) {
            ...
            if (Utiilties.isOnline(PendingPfmsStudentList.this)) {
                Toast.makeText(this, "Please wait downloading class", Toast.LENGTH_SHORT).show();
                new LoadClassList().execute();

            } else {

                AlertDialog.Builder ab = new AlertDialog.Builder(PendingPfmsStudentList.this);
    ...
    private class LoadClassList extends AsyncTask<String, Void, ArrayList<CLASSLIST>> {
        ...
        protected void onPreExecute() {
            pd2 = new ProgressDialog(PendingPfmsStudentList.this);
        ...
        protected void onPostExecute(ArrayList<CLASSLIST> result) {
            ...
            if (result != null) {
                if (result.size() > 0) {
                    ...
                    if(i>0){
                        Toast.makeText(PendingPfmsStudentList.this, "Class Data downloaded", Toast.LENGTH_SHORT).show();
                        ...
                        Toast.makeText(PendingPfmsStudentList.this, "not inserted", Toast.LENGTH_SHORT).show();
                    }
        ...
        protected void onPostExecute(ArrayList<SESSIONLIST> result) {
            ...
            if (result != null) {
                if (result.size() > 0) {
                    ...
                    if(i>0){
                        Toast.makeText(PendingPfmsStudentList.this, "Section Data downloaded...", Toast.LENGTH_SHORT).show();
                        ClassList = helper.getClassList();
                        if(ClassList.size()> 0){
                            loadSessionList();
                        }
    ...
    public void searchStdList()
    {
        ...
        benfiLists = dataBaseHelper.PendingPFmsStudentList(_varClass_Id,_varSession_Id);
        ...
        if (benfiLists.size() == 0) {
            ...
            recyclerViewAdapter = new SerBList(PendingPfmsStudentList.this, benfiLists);
